    ...
    setting: {
      chatService: class ChatService {
        ...
        static useSocketArea(socket, domName) {
          ...
          function initRemainHTML(domName) {
            const chatBox = document.getElementById(`${domName}`);
          ...
          function commonHTML1(domName) {
            ...
            // chatBox에 적용
            // 공통 채팅창 적용
            const chatBoxDiv = document.getElementById(`${domName}`);
            if (chatBoxDiv !== null) {
      ...
      client: class Client {
        constructor(domName, clientId, api) {
          this.domName = domName;
          this.clientId = clientId;
          this.api = api;
        ...
        async init(userId) {
          try {
            ...
            function drawingManagerChat(domName, data) {
              if (!domName)
                throw new SmpChatError("관리자 채팅 domId를 설정해주세요.");
              Manager.commonHTML(domName);
              drawManagerHTML(domName);

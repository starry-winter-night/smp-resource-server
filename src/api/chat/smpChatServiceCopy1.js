(function (w) {
  ...
  const clientHTML = () => {
    ...
    if (clientChatBoxDiv) {
      ...
      clientChatBoxDiv.style.setProperty("position", "fixed", "important");
      ...
      chatBoxImg.onclick = () => {
        ...
        clientChatBoxDiv.style.setProperty("height", "550px", "important");
      };
    }
  ...
  const managerHTML = () => {
    // 관리자의 채팅창
    const chatBox = document.getElementById("smpManagerChat");
    if (chatBox) {
      const chatBoxImg = document.createElement("IMG");
      chatBoxImg.setAttribute(
        "src",
    ...
    //   const dialogDiv = document.getElementById("smp_dialogDiv");
    //   const footerDiv = document.getElementById("smp_footerDiv");
    //   const empty = isEmpty(messageInput.value);
    //   const messageButton = document.getElementById("smp_messageButton");
    //   focusProcess(dialogDiv, messageInput, footerDiv, messageButton);
    ...
    document.styleSheets[0].addRule(".smp_radioDiv p", "display:inline-block;");
    document.styleSheets[0].addRule(".smp_radioDiv p", "font-size:15px;");
    document.styleSheets[0].addRule(".smp_radioDiv p", "font-weight:bold;");

    document.styleSheets[0].addRule(
      ".smp_switchSpan:before",
      "position:absolute;"
    );
    document.styleSheets[0].addRule(".smp_switchSpan:before", 'content:"";');
    document.styleSheets[0].addRule(".smp_switchSpan:before", "height:22px;");
    document.styleSheets[0].addRule(".smp_switchSpan:before", "width:22px;");
    document.styleSheets[0].addRule(".smp_switchSpan:before", "left:4px;");
    document.styleSheets[0].addRule(".smp_switchSpan:before", "bottom:4px;");
    document.styleSheets[0].addRule(
      ".smp_switchSpan:before",
    ...
    document.styleSheets[0].addRule(".smp_round", "border-radius: 30px;");
    document.styleSheets[0].addRule(".smp_round:before", "border-radius: 50%;");
  ...
  const switchButtonCtrl = (state) => {
    if (state === "off") {
      document.getElementById("smp_switchSpan").style.backgroundColor = "#ccc";
      document.styleSheets[0].addRule(
        ".smp_switchSpan:before",
        "position:absolute;"
      );
      ...
      document.getElementById("smp_switchOffP").style.display = "inline";
      document.getElementById("smp_switchOnP").style.display = "none";
    }
    if (state === "on") {
      document.getElementById("smp_switchSpan").style.backgroundColor =
        "#2196F3";
      ...
      document.getElementById("smp_switchOffP").style.display = "none";
      document.getElementById("smp_switchOnP").style.display = "inline";
    }
  };
  ...
  const createPreview = (data) => {
    ...
    const listUserNameText = document.createTextNode(name);
    listUserName.appendChild(listUserNameText);
    list.appendChild(listUserName);
    listUserName.id = `smp_chatUser:${name}`;
    ...
    const listDateText = document.createTextNode(time);
    listDate.appendChild(listDateText);
    list.appendChild(listDate);
    listDate.id = `smp_chatDate:${name}`;
    ...
    const listContentText = document.createTextNode(message);
    listContent.appendChild(listContentText);
    list.appendChild(listContent);
    listContent.id = `smp_chatContent:${name}`;
